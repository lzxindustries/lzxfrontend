...
export async function getAllPatches(context: AppLoadContext) {
    ...
    const patch_videos_data = await getDataCollection(context, "PatchVideo") as PatchVideoInterface[];
    const patch_modules_data = await getDataCollection(context, "PatchModule") as PatchModuleInterface[];
    const modules_data = await getDataCollection(context, "PatchModule") as ModuleInterface[];
        ...
        patch_videos_data.map((patch_video) => {
            patch_video.video == patch_data._id ?
                videos_data.map((video) => {
                    video._id == patch_video.video ? patch_view.videos.push(
                        {
                            name: video.name,
        ...
        patch_modules_data.map((patch_module) => {
            patch_module.module == patch_data._id ?
                modules_data.map((module) => {
                    module._id == patch_module.module ? patch_view.modules.push(
                        {
                            name: module.name
                        }) : null
                }) : null
