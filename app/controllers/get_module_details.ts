...
export async function getModuleDetails(context: AppLoadContext, id: string) {
    ...
    const controls_data = await getDataCollection(context, "ModuleControl", [{ $limit: 1024 }]) as ModuleControlInterface[]
    const connectors_data = await getDataCollection(context, "ModuleConnector", [{ $limit: 1024 }]) as ModuleConnectorInterface[]
