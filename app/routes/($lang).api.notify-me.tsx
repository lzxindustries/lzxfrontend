...
export async function action({request, context}: ActionFunctionArgs) {
  ...
  if (SITE_DECISIONS.backInStockProvider === 'klaviyo') {
    const apiKey = (context.env as any).KLAVIYO_PRIVATE_API_KEY as string | undefined;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('Klaviyo notify-me failed', {status: response.status, details, handle});
        return json(
          {ok: false, message: 'Unable to subscribe right now. Please try again soon.'},
          {status: 502},
        );
      }

      return json({ok: true, message: 'You are subscribed. We will email when it is back in stock.'});
    } catch (error) {
  ...
  return json({ok: false, message: 'Notify-me provider is not configured.'}, {status: 500});
}
