...
Keychain does not have a video sync input. Low frequency transitions can happen in the middle of a frame, causing visible frame tearing. To cause transitions to happen in the vertical interval between frames or fields, you'll need an audio _sample and hold_ module and a trigger signal that goes high at the start of each frame.
