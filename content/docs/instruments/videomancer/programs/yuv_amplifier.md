...
_YUV Amplifier shaping independent gain, offset, and inversion across all three video channels to transform color and contrast in real time._
...
The name is literal and descriptive. **_YUV_** refers to the color space the program operates in: Y for luminance, U for blue-difference chrominance, and V for red-difference chrominance. **_Amplifier_** describes the core function: a **_processing amplifier_**, or proc amp, that scales and shifts each channel. In broadcast engineering, a proc amp is the standard tool for adjusting video levels. YUV Amplifier brings that capability to Videomancer with per-channel precision.
...
_Videomancer's front panel with YUV Amplifier active. Knobs 1–6 (top two rows of left cluster), Toggle switches 7–11 (bottom row of left cluster), Fader 12 (right side)._
...
:::tip
Adjusting **U Offset** and **V Offset** together is equivalent to a **_white balance_** correction. Small shifts can compensate for the color temperature of a light source, matching the look between different cameras or scenes.
:::
...
**Y Invert** flips the luminance channel by applying a bitwise complement. When set to **On**, every pixel's brightness is reversed: black becomes white and white becomes black, producing a photographic negative effect on the luminance. Inversion is the **_first_** processing step: it happens before gain and offset are applied, so all downstream processing operates on the inverted signal. This means **Y Gain** and **Y Offset** will amplify and shift the inverted values.
...
:::tip
**Fade Amount** is a **_wet/dry mix_** for the entire program. Use it as a performance control to smoothly blend between your processed look and a solid color, or to dial back the intensity of extreme gain and offset settings without changing them individually.
:::
...
A **_processing amplifier_** applies two operations to a signal: gain (multiplication) and offset (addition). Gain controls the contrast: how far values spread from the center point. Offset controls the brightness: shifting all values uniformly up or down. The mathematical formula is:
...
Each channel can be independently inverted before entering the proc amp. Inversion applies a **_bitwise complement_**: each bit is flipped from 0 to 1 or 1 to 0. For a 10-bit value, this transforms value _n_ into 1023 − _n_. The result is a mirror image of the tonal range. Because inversion happens _before_ the proc amp, the gain and offset stages operate on the inverted signal, which means their behavior is reversed relative to the source.
...
_Brightness and Contrast — simulated result across source images._
...
_Color Temperature Shift — simulated result across source images._
...
_Inverted Fade Performance — simulated result across source images._
...
- **Bitwise Complement**: Flipping every bit in a binary number; for a 10-bit value _n_, this produces 1023 − _n_.
...
- **Interpolator**: A circuit that computes a weighted blend between two values based on a mix parameter _t_.
