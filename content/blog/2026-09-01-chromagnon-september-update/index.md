---
slug: chromagnon-september-update
title: 'Chromagnon Update: September 2026'
authors: [lars]
tags: [chromagnon, manufacturing]
draft: true
# image: ./chromagnon-september.jpg
---

Monthly Chromagnon update for September 2026.

<!--truncate-->

<!--
CONTEXT: This is targeted as the Ship Unit #1 post.
If the schedule has held, the first batch should be in assembly/QC or shipping.
This is the post customers have been waiting YEARS for. Make it count.

IF UNIT #1 HAS SHIPPED:
-->

## Ship Unit #1

<!--
THE MILESTONE. Report:
- Unit #1 has shipped. Date, destination (general — don't share customer details without permission).
- Photo of the unit being packed / on the shipping bench
- What's in the box (contents list with photo)
- First impressions — any last-minute discoveries during final QC?

Reflect briefly on the journey — but keep it tight. This is a celebration, not a eulogy.
Don't re-litigate the delays. Point forward.
-->

<!--
IF UNIT #1 HAS NOT SHIPPED:
Be completely transparent about why. Update the timeline.
Do not spin. Do not minimize. State facts, state the revised date, state what's blocking.
-->

## Fulfillment Begins

<!--
Concrete fulfillment logistics:
- How many units in the first shipping batch?
- How many shipped this month / shipping this week?
- When does the next batch ship?
- Revised estimate: when will all 700 pre-orders be fulfilled?
- Current shipping rate vs. target of 150-200/month

Include shipping stats:
- Units shipped to date: __
- Units in QC pipeline: __
- Next batch ships: __
- Estimated completion of all pre-orders: __
-->

**Fulfillment tracker:**

| Batch   | Units | Status | Ship Date |
| ------- | ----- | ------ | --------- |
| Batch 1 |       |        |           |
| Batch 2 |       |        |           |
| Batch 3 |       |        |           |

## First Customer Reactions

<!--
If any early recipients have shared reactions, photos, or videos — feature them here.
This is powerful social proof that Chromagnon is REAL and SHIPPING.
Reach out to the first few customers and ask if they'd be willing to share.
-->

## Ongoing Updates

<!--
Commit to the monthly cadence through fulfillment completion:
- September onward: monthly batch shipping reports
- Each update includes: units shipped, units remaining, next batch date
- Customer portal link for individual queue tracking
-->

---

## Media Shot List (Internal — Remove Before Publishing)

1. **chromagnon-august.jpg** — THE photo if shipping: first completed unit, boxed and ready. If not shipping: production line progress photo.
2. **chromagnon-box-contents.jpg** — Unboxing layout of everything included
3. **chromagnon-assembly-line.jpg** — Production batch being assembled
4. **chromagnon-qc-pass.jpg** — Unit passing final QC (oscilloscope showing clean output)
5. **chromagnon-shipping-bench.jpg** — Units packed and labeled for shipping
