...
1. **Tier A — System coverage (CI-friendly)**  
   Every hub slug has: shippable manual _or_ explicit exception; `seo.json` not empty where the product is ACTIVE; `SUPPORT_MANIFEST` slugs either have a support file with navigational FAQ _or_ are listed in the audit as “intentionally empty” (legacy-only products).  
...
2. **Tier B — Editorial**  
   Manuals are technically accurate, on-voice per `content/docs/WRITING_STYLE_GUIDE.md`, and not redundant with Specs. Support FAQs answer _real_ recurring questions.  
...
- **Support** — every manifest row either has a `.md` file _or_ is explicitly deferred with product rationale (not silent drift).
